  ...
  return (
    <div className="p-6 rounded-2xl bg-slate-800 border border-slate-700 hover:border-cyan-500 transition-all duration-300 shadow-xl group">
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-4 sm:gap-0 mb-4">
        <h2 className="text-2xl sm:text-3xl font-bold text-cyan-400 group-hover:text-cyan-300 break-words">{term}</h2>
        <span className={`px-3 py-1 text-[10px] sm:text-xs font-semibold uppercase tracking-wider rounded-full ${getCategoryColor(category)}`}>
          {category}
        </span>
      </div>
      <p className="text-slate-300 leading-relaxed italic text-sm sm:text-base break-words">
        "{definition}"
      </p>
    </div>
  );
